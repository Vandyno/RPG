...
class ActionContext:
	...
	func interact_npc(entity) -> void:
		_interact_npc.call(entity)
...
static func _poi_should_offer_inspect(ctx: ActionContext, entity) -> bool:
	...
	return (
		...
		and not _available_poi_actions(ctx, entity).is_empty()
	)
