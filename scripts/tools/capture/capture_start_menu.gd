...
const WIDTH := 1152
const HEIGHT := 648
const OUTPUT_PATH := "res://reports/start_menu/start_menu.png"
...
func _capture() -> void:
	...
	if image == null:
		printerr("Could not capture start menu.")
		quit(1)
		return
	var absolute_path := ProjectSettings.globalize_path(OUTPUT_PATH)
	var make_error := DirAccess.make_dir_recursive_absolute(absolute_path.get_base_dir())
	if make_error != OK:
		printerr("Could not create start-menu capture directory: %s" % error_string(make_error))
		quit(1)
		return
	var save_error := CaptureSheetHelper.save_png_image(image, absolute_path)
	if save_error != OK:
		printerr("Could not save start-menu capture: %s" % error_string(save_error))
		quit(1)
		return
	print("Wrote start-menu capture to %s" % absolute_path)
	quit()
