...
func damage_entity(
	...
	var raw_counter_damage := (
		...
		else VariantFields.non_negative_int_field(entity.data, "attack_damage", DEFAULT_ENEMY_DAMAGE)
	)
	var counter_damage := (
...
func load_save_data(data: Dictionary) -> void:
	...
	var loaded_health := VariantFields.dictionary(data.get("health_by_entity_id", {}))
	for entity_id in loaded_health:
		var key := String(entity_id)
		var health_value: Variant = loaded_health[entity_id]
		if not key.is_empty() and VariantFields.is_number(health_value) and int(health_value) > 0:
...
func _clamped_entity_health(entity_id: String, max_health: int) -> int:
	...
	var stored := int(stored_value) if VariantFields.is_number(stored_value) else safe_max
	return clampi(stored, 0, safe_max)


func _max_health_for_entity(entity) -> int:
	return VariantFields.positive_int_field(entity.data, "max_health", DEFAULT_MAX_HEALTH)


func _attack_damage_for_entity(entity) -> int:
	var base_damage := VariantFields.positive_int_field(
		entity.data, "damage_taken_per_hit", player_damage
	)
	var bonus := 0
