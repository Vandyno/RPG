...
func apply_status(status_id: String, charges_override: int = 0) -> bool:
	...
	var charges := (
		...
		else VariantFields.non_negative_int_field(definition, "attack_charges", 0)
	)
	if charges <= 0:
		return false
...
func get_player_damage_bonus() -> int:
	...
	for status_id in active_statuses:
		...
		bonus += VariantFields.non_negative_int_field(definition, "damage_bonus", 0)
	return bonus
...
func guarded_counter_multiplier(base_multiplier: float) -> float:
	...
	for status_id in active_statuses:
		...
		if (
			...
			and VariantFields.is_number(definition["guard_counter_multiplier"])
		):
...
func get_remaining_charges(status_id: String) -> int:
	var state := VariantFields.dictionary(active_statuses.get(status_id, {}))
	return (
		maxi(0, int(state.get("charges", 0)))
		if VariantFields.is_number(state.get("charges", 0))
		else 0
	)
...
func get_details() -> String:
	...
	for status_id in _sorted_status_ids():
		...
		var bonus := VariantFields.non_negative_int_field(definition, "damage_bonus", 0)
...
func load_save_data(data: Dictionary) -> void:
	...
	for entry in VariantFields.array(data.get("active", [])):
		var status := VariantFields.dictionary(entry)
		var status_id := String(status.get("status_id", ""))
		var charges := VariantFields.non_negative_int_field(status, "charges", 0)
