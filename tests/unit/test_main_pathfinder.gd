...
func test_pathfinder_routes_smooth_waypoints_around_blocked_tiles() -> void:
	...
	var path := MainPathfinder.path_to(main, player.global_position, destination)
...
func test_pathfinder_rejects_blocked_destination() -> void:
	...
	assert_true(MainPathfinder.path_to(main, player.global_position, destination).is_empty())
...
func test_pathfinder_can_approach_blocked_target_within_interaction_range() -> void:
	...
	var path := MainPathfinder.approach_path_to(main, player.global_position, target, 72.0)
