...
func test_target_cycle_touch_next_and_picker_uses_spawn_interactables() -> void:
	...
	assert_true(main.get_debug_state()["navigation"].contains("NW 5.4t Road Thug"))
...
func test_rest_interaction_heals_player() -> void:
	...
	main.hud.set_systems_tab("world")
...
func test_save_and_load_hide_open_systems_panel_while_running() -> void:
	...
	main.hud.show_systems_panel("world")
...
func test_full_spawn_yard_system_loop() -> void:
	...
	assert_true(main.hud.systems_body_label.text.contains("Progression: Level 2"))
	...
	assert_eq((main.hud.systems_action_list.get_child(0) as Button).text, "No actions")
	assert_true((main.hud.systems_action_list.get_child(0) as Button).disabled)
	assert_true(main.time.advance_hours(12))
	main.hud.refresh()
	...
	assert_true(main.chunks.is_object_opened("object_road_cache", Vector2i(3, -5)))
...
func test_main_save_load_restores_spawn_yard_system_state() -> void:
	...
	main.chunks.mark_entity_removed("enemy_road_thug", Vector2i(-5, -2))
