...
Owns known spell assignments, prepared ability slots, spell owner IDs, and
spell save/load. It supports owner-scoped loadouts so player and NPC humanoids
can share the same spell contract instead of relying on a player-only spell
list.
...
The Journal and Quest tabs may show navigation summaries generated by
...
Spell assignment is owner-scoped in `SpellManager`; the player HUD currently
renders the active player's assigned ability slots.
